...
{
    //办公建筑
    public class Office : Building, ITrans
    {
        #region 变量
        //办公平面类型，按走廊位置划分
        ...
        //内部变量
        public OfficeType officeType;//办公走廊类型
        public double heightPerFloor = 6;
        public double columnSpan = 8;//房间进深
        public double unitWidth;//办公每层作为一个单体，该单体的短边长度
        public double unitMaxArea;//每层办公最大面积
        public double unitCount;//所需办公单元数量
        public List<OfficeUnits> officeUnits = new List<OfficeUnits>();//办公单体组
        ...
        //构造函数
        public Office(double area, Point3d origin)
        {
            StaticObject.office = this;
            buildingType = BuildingType.办公;
            //外部数据初始化
            this.area = area;
            this.showOrigin = origin;
            this.heightPerFloor = 6;
            //判断办公平面形式
            GetOfficeType();
            //创建办公单元
            CreateOfficeUnit();
        ...
        public void CreateOfficeUnit()
        {
            ...
            {
                officeUnits.Add(new OfficeUnits(i));
            }
            //各层叠落
            UnitLayout();
            // 获取场馆边界
        ...
        public void GetOfficeType()
        {
            if (Tool.GetBool())//外廊
            {
                officeType = OfficeType.外廊;
            }
            else//内廊
            {
                officeType = OfficeType.内廊;
            }
        }
        //排列办公单体
        public void UnitLayout()
        {
            ...
            for (int i = 0; i < unitCount; i++)
            {
                officeUnits[i].officeUnit.Transform(Transform.Translation(Vector3d.ZAxis * heightPerFloor * i));
            }
        }
        // 获取场馆边界
        public void GetBoundary()
        {
            //创建场馆单体边界
            #region
            BoundingBox tempBox = officeUnits[0].officeUnit.Boundingbox;
            if (unitCount > 1)
            {
                for (int i = 1; i < unitCount; i++)
                {
                    tempBox = BoundingBox.Union(tempBox, officeUnits[i].officeUnit.Boundingbox);
                }
            }
        ...
        public void Move(Transform trans)
        {
            ...
            for (int i = 0; i < unitCount; i++)
            {
                officeUnits[i].officeUnit.Transform(trans);
            }
        ...
        public void MustRotate(double angle)
        {
            ...
            for (int i = 0; i < unitCount; i++)
            {
                officeUnits[i].officeUnit.Transform(transform);
            }
        ...
        public SimplifiedBuilding CreateSimplifiedBuilding()
        {
            ...
            {
                gymAuxiliaries.Add(officeUnits[i].officeUnit);
            }
            //构造新的简化场馆
